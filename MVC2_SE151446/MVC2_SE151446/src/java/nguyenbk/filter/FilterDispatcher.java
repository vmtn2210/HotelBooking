...
public class FilterDispatcher implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            ...
            int firstParam = uri.indexOf("?");
            if (firstParam != -1) {
                resource = uri.substring(lastIndex + 1, firstParam);
            } else {
                resource = uri.substring(lastIndex + 1);
            }
            if (resource.equals("addBookController")) {
                String button = req.getParameter("btAction");
                if (button.equals("Go Shopping")) {
                    resource = "shoppingPage";
                } else if (button.equals("Remove Your Choice Items")) {
                    resource = "removeItemFromCart";
                }
            }
            //get site mapping
